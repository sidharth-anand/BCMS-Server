...
async function getNotifications(uid, callback) {
    try {
        const notifications = await db.query("SELECT p.pid, p.title, p.posted_in, c.name, p.created_at, p.updated_at FROM bcms_notification n, bcms_post p, bcms_course c WHERE n.uid = $1 AND p.pid = n.pid AND p.posted_in = c.cid", [uid])
